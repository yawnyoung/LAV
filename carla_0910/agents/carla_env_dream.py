...
class RouteScenarioManager(object):
    
    def __init__(self, timeout=200000000., debug=False) -> None:
        self.manager = ScenarioManager(timeout, debug)
        self._agent_watchdog = Watchdog(int(float(timeout)))
        self.sensors = None
        self.client = carla.Client('localhost', 2000)
        self.traffic_manager_port = 8000
        self.traffic_manager_seed = 0
        self.traffic_manager = self.client.get_trafficmanager(int(self.traffic_manager_port))
        self.ego_vehicles = []
    ...
    def _load_and_wait_for_world(self, town, ego_vehicles=None):
        ...
        CarlaDataProvider.set_traffic_manager_port(int(self.traffic_manager_port))

        self.traffic_manager.set_synchronous_mode(True)
        self.traffic_manager.set_random_device_seed(int(self.traffic_manager_seed))
    ...
    def _prepare_ego_vehicles(self, ego_vehicles, wait_for_ego_vehicles=False):
        ...
        if not wait_for_ego_vehicles:
            print('size of ego vehicles {}'.format(len(ego_vehicles)))
            for vehicle in ego_vehicles:
                ...
                self.collision_sensor = self.world.spawn_actor(bp, carla.Transform(), attach_to=self.ego_vehicles[-1])
                self.collision_sensor.listen(lambda event: self._on_collision(event))
                print('add collision sensor to ego')
            ...
            for i, _ in enumerate(self.ego_vehicles):
                ...
                bp = self.world.get_blueprint_library().find('sensor.other.collision')
                self.collision_sensor = self.world.spawn_actor(bp, carla.Transform(), attach_to=self.ego_vehicles[i])
                self.collision_sensor.listen(lambda event: self._on_collision(event))

        # sync state
        CarlaDataProvider.get_world().tick()

    def _load_and_run_scenario(self, agent_instace, config):
        """
        ...
        try:
            ...
            self.agent_instance = agent_instace
            ...
            if not self.sensors:
                ...
                AgentWrapper.validate_sensor_configuration(self.sensors, track, track)
        ...
        try:
            self._load_and_wait_for_world(config.town, config.ego_vehicles)
            self._prepare_ego_vehicles(config.ego_vehicles, False)
            scenario = RouteScenario(world=self.world, config=config, debug_mode=False)
            for ego_vehicle in scenario.ego_vehicles:
                bp = self.world.get_blueprint_library().find('sensor.other.collision')
                self.collision_sensor = self.world.spawn_actor(bp, carla.Transform(), attach_to=ego_vehicle)
                self.collision_sensor.listen(lambda event: self._on_collision(event))
                print('add collision sensor to ego')
...
class CarlaEnv(object):
    ...
    def __init__(self,
        ...
        self.port = port
        self.rs_manager = RouteScenarioManager()
        self.route_indexer = RouteIndexer(os.environ['ROUTES'], os.environ['SCENARIOS'], int(os.environ['REPETITIONS']), '')
        self.episode_ego_wps = []
        ...
        self._observation_space = DotMap()
        self._observation_space.shape = (rl_image_size, rl_image_size, 3)
        self._observation_space.dtype = np.dtype(np.uint8)
    ...
    def simulate(self, agent_instance, steps=0, episodes=0):
        # run
        step, episode = 0, 0
        done = False
        scenario_succ = False
        while self.route_indexer.peek() and ((steps and step < steps) or (episodes and episode < episodes)):
            config = self.route_indexer.next()
            self.rs_manager._load_and_run_scenario(agent_instance, config)
            self.map = self.rs_manager.world.get_map()

            self.rs_manager.manager.start_system_time = time.time()
            self.rs_manager.manager.start_game_time = GameTime.get_time()
            self.rs_manager.manager._watchdog.start()
            self.rs_manager.manager._running = True
            
            count_collision = 0
            vehicle_wps = []
            collision_idx = -1

            while self.rs_manager.manager._running and ((steps and step < steps) or (episodes and episode < episodes)):
                action = self.simulator_step()
                if not action:
                    print('ERROR: simulator step error')
                    return
                info = {}
                info['reason_episode_ended'] = ''
                vehicle_wps.append(self.rs_manager.ego_vehicle.get_location())
                dist_from_center, vel_s, speed, done, info = self.dist_from_center_lane(self.rs_manager.ego_vehicle, info)
                collision_intensities_during_last_time_step = sum(self.rs_manager._collision_intensities_during_last_time_step)
                if len(self.rs_manager._collision_intensities_during_last_time_step) > 0:
                    count_collision += 1
                if count_collision == 1:
                    collision_idx = len(vehicle_wps) - 1
                self.rs_manager._collision_intensities_during_last_time_step.clear()  # clear it ready for next time step
                ...
                reward = vel_s * dt - collision_cost - abs(action.steer)  # doesn't work if 0.001 cost collisions
                info['crash_intensity'] = collision_intensities_during_last_time_step
                info['steer'] = action.steer
                info['brake'] = action.brake
                ...
                if count_collision > 10:
                    print("Episode fail: too much collision {}".format(self.return_))
                    info['reason_episode_ended'] = 'collision'
                    done = True

                if done:
                    if info['reason_episode_ended'] in ['stuck', 'collision', 'off_road']:
                        print('should return')
                        if collision_idx >= 0:
                            return_idx = max(0, collision_idx - 10)
                            return_loc = vehicle_wps[return_idx]
                            return_wp = self.map.get_waypoint(return_loc).transform
                            print('current location {} set transform {} collision idx {} wps size {}'.format(vehicle_wps[-1], return_wp, collision_idx, len(vehicle_wps)))
                            self.rs_manager.ego_vehicle.set_transform(return_wp)
                            self.rs_manager.world.tick()
    ...
    def simulator_step(self):
        ...
        if timestamp:
            return self.rs_manager.manager._tick_scenario_customized(timestamp) 
        return None
    ...
    def step(self, action=None):
        ...
        info = {}
        info['reason_episode_ended'] = ''
        self.episode_ego_wps.append(self.rs_manager.ego_vehicle.get_location())
        dist_from_center, vel_s, speed, done, info = self.dist_from_center_lane(self.rs_manager.ego_vehicle, info)
        collision_intensities_during_last_time_step = sum(self.rs_manager._collision_intensities_during_last_time_step)
        if len(self.rs_manager._collision_intensities_during_last_time_step) > 0:
            count_collision += 1
        if count_collision == 1:
            collision_idx = len(self.episode_ego_wps) - 1
        self.rs_manager._collision_intensities_during_last_time_step.clear()  # clear it ready for next time step
        assert collision_intensities_during_last_time_step >= 0.
        dt = 0.05
        collision_cost = 0.0001 * collision_intensities_during_last_time_step
    ...
    def _simulator_step(self, action, dt=0.05):
        ...
        # rgb = np.resize(rgb,(self.rl_image_size, self.rl_image_size, 3))
        ...
        assert next_obs.shape == self._observation_space.shape
        if self.count >= self._max_episode_steps:
            print("Episode success: I've reached the episode horizon ({}).".format(self._max_episode_steps))
            info['reason_episode_ended'] = 'success'
            done = True
    ...
    def run(self, agent_instance):
        ...
        while self.route_indexer.peek():
            ...
            self.route_indexer.save_state(os.environ['CHECKPOINT_ENDPOINT'])
