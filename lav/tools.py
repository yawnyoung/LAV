...
def simulate_lav(agent, env, steps=0, episodes=0):
  step, episode = 0, 0
  route_indexer = env.route_indexer
  rs_manager = env.rs_manager
  while route_indexer.peek() and ((steps and step < steps) or (episodes and episode < episodes)):
    config = route_indexer.next()
    rs_manager._load_and_run_scenario(agent, config)
    map = rs_manager.world.get_map()

    rs_manager.manager.start_system_time = time.time()
    rs_manager.manager.start_game_time = GameTime.get_time()
    rs_manager.manager._watchdog.start()
    rs_manager.manager._running = True
    ...
    # while rs_manager.manager._running and ((steps and step < steps) or (episodes and episode < episodes)):
